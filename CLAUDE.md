...
Credentials are loaded only from `~/.config/coros-mcp/.env` (optional) via `coros_mcp/auth/env.py` — not from project or vault `.env` files. For git-synced Obsidian vaults, use `coros-mcp auth` and project-scoped MCP config without `COROS_PASSWORD` in env.
...
- **`coros_mcp/auth/`**: Token storage abstraction. Priority chain: env var → encrypted file → keyring. `encrypted_store.py` uses AES-256-GCM with a machine-bound key (machine *binding* against off-machine leaks, not protection from local attackers — that comes from 0600 file perms); `keyring_store.py` wraps the system keyring. `env.py` loads optional credentials from `~/.config/coros-mcp/.env` only (never from project/vault cwd).
